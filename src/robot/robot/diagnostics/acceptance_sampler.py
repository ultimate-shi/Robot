...
class AcceptanceSampler(Node):
    ...
    def _save_request(self, msg):
        ...
        if image_msg is None or depth_msg is None or info_msg is None:
            self._status(False, '图像、深度或 CameraInfo 尚未全部就绪')
            return
        try:
            ...
            self._status(False, f'保存验收样本失败: {exc}')
            return
        self._status(True, '验收样本已保存', directory=directory)
    ...
    def _write_sample(self, image_msg, depth_msg, info_msg, detections,
        ...
        if not cv2.imwrite(
                os.path.join(directory, 'left_rect.jpg'), image,
                [cv2.IMWRITE_JPEG_QUALITY, self.jpeg_quality]):
            raise OSError('写入左目图像失败')
