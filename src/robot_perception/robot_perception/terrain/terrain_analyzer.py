#!/usr/bin/env python3
"""
使用方法：由 robot_perception/stereo_perception.launch.py 启动地形分析节点。
...
from robot_perception.terrain.terrain_heightmap import TerrainHeightmap
from robot_perception.terrain.terrain_physics import TerrainPhysics
...
class TerrainAnalyzerNode(Node):
    ...
    def __init__(self):
        ...
        self.typed_status_pub = self.create_publisher(
            TerrainState, '/perception/terrain_state', 10)
    ...
    def timer_callback(self):
        ...
        typed.is_blocked = bool(status['is_blocked'])
        typed.block_reason = str(status['block_reason'])
        typed.traversability = float(status['traversability'])
        typed.slip_factor = float(status['slip_factor'])
        typed.body_z = float(status['body_z'])
        typed.roll = float(status['roll'])
        typed.pitch = float(status['pitch'])
        typed.step_blocked = bool(status['step_blocked'])
        typed.dropoff_blocked = bool(status['dropoff_blocked'])
