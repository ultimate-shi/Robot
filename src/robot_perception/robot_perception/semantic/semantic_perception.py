...
class SemanticPerception(Node):
    ...
    def __init__(self):
        ...
        defaults = {
            ...
            'legacy_output_topic': '/perception/semantic_detections_json',
        ...
        self.legacy_output_pub = self.create_publisher(
    ...
    def _infer(self, image_msg, depth_msg, info_msg):
        ...
        if not success:
            raise ValueError('语义识别图像 JPEG 编码失败')
    ...
    def _detect_service(self, request, response):
        ...
        typed = self.latest_typed
        if self.latest_payload is None:
            response.success = False
            response.error_code = 'NO_RESULT'
            response.message = '尚无检测结果，已安排识别最新画面'
            response.detections = typed
            return response
        filtered = SemanticDetectionArray()
        filtered.header = typed.header
        filtered.image_width = typed.image_width
        filtered.image_height = typed.image_height
        filtered.model = typed.model
        filtered.latency_ms = typed.latency_ms
        classes = {item.lower() for item in request.class_filter}
        for item in typed.detections:
            if classes and item.class_name.lower() not in classes:
                continue
            if item.confidence < request.min_confidence:
                continue
            if request.require_map_position and not item.has_map_position:
                continue
            filtered.detections.append(item)
        response.success = True
        response.error_code = ''
        response.message = f'返回 {len(filtered.detections)} 个目标'
        response.detections = filtered
        return response

    def _set_mode_service(self, request, response):
        """切换按需/持续检测；目标过滤由任务层根据稳定 ID 完成。"""
        if request.mode == SetDetectionMode.Request.ON_DEMAND:
            self.detection_mode = 'on_demand'
        elif request.mode == SetDetectionMode.Request.CONTINUOUS:
            self.detection_mode = 'continuous'
        else:
            response.success = False
            response.message = '不支持的检测模式'
            return response
        response.success = True
        response.message = f'检测模式已切换为 {self.detection_mode}'
        return response

    def _publish_payload(self, payload):
        """同时发布最终类型化接口和迁移期 JSON 兼容接口。"""
        typed = SemanticDetectionArray()
        typed.header.stamp.sec = int(payload['stamp']['sec'])
        typed.header.stamp.nanosec = int(payload['stamp']['nanosec'])
        typed.header.frame_id = self.map_frame
        typed.image_width = int(payload['image']['width'])
        typed.image_height = int(payload['image']['height'])
        typed.model = str(payload['model'])
        typed.latency_ms = float(payload['latency_ms'])
        for item in payload['detections']:
            msg = SemanticDetection()
            msg.id = str(item['id'])
            msg.class_name = str(item['class_name'])
            msg.label_zh = str(item['label_zh'])
            msg.confidence = float(item['confidence'])
            box = item['bbox']
            msg.bbox_x_min, msg.bbox_y_min = int(box[0]), int(box[1])
            msg.bbox_x_max, msg.bbox_y_max = int(box[2]), int(box[3])
            msg.has_depth = item['distance'] is not None
            msg.distance = float(item['distance'] or 0.0)
            point = item['map_position']
            msg.has_map_position = point is not None
            if point is not None:
                msg.map_position.header = typed.header
                msg.map_position.point.x = float(point['x'])
                msg.map_position.point.y = float(point['y'])
                msg.map_position.point.z = float(point['z'])
            typed.detections.append(msg)
