...
# 预览与遮挡检查：bash scripts/stereo/run_stereo_calibration.sh preview
# 正式标定：bash scripts/stereo/run_stereo_calibration.sh calibrate <实测格子边长米>
...
if [[ "${MODE}" != "preview" && "${MODE}" != "calibrate" ]]; then
  echo "用法：" >&2
  echo "  bash scripts/stereo/run_stereo_calibration.sh preview" >&2
  echo "  bash scripts/stereo/run_stereo_calibration.sh calibrate <实测格子边长米>" >&2
  exit 2
fi
...
if ! docker image inspect robot-jazzy:local >/dev/null 2>&1; then
  echo "未找到 robot-jazzy:local，请先运行 bash scripts/docker/build_jazzy_image.sh" >&2
  exit 1
fi

if [[ ! -e /dev/stereo_camera ]]; then
  echo "未找到 /dev/stereo_camera。请先执行：" >&2
  echo "  sudo bash scripts/stereo/install_stereo_camera_udev.sh" >&2
  echo "然后重新插拔相机。" >&2
  exit 1
fi
...
docker run --rm --interactive --tty \
  ...
  bash -lc '
    ...
    ros2 launch robot_perception stereo_camera.launch.py \
      calibration_mode:=true apply_auto_camera_controls:=false \
