...
# 典型命令：sudo bash scripts/install_jazzy_docker.sh
...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "请使用 root 权限运行：sudo bash scripts/install_jazzy_docker.sh" >&2
  exit 1
fi
...
echo "Docker 安装完成。重新登录后执行：bash scripts/build_jazzy_image.sh"
